...
main(int argc, char** argv) {
    ...
    for (const auto& metric : metrics) {
        ...
        for (const auto& algo : algos) {
            ...
            // q_cut values to test (0 = no pruning, then various cut thresholds)
            // Based on Seismic paper: top 10-15 query terms capture 75%+ of query mass
            std::vector<int> q_cut_values = {0};  // Baseline: no pruning
            if (algo == "DAAT_MAXSCORE_V2") {
                q_cut_values = {0, 10, 15, 20, 25, 30};  // Test various q_cut values
            }

            for (int q_cut : q_cut_values) {
                search_conf["q_cut"] = q_cut;

                // Run benchmark
                printf("\n[Running Search Benchmark] q_cut=%d\n", q_cut);
                ...
                printf("\n[Results for %s (%s) q_cut=%d]\n", algo.c_str(), metric.c_str(), q_cut);
